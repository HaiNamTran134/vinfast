...
if (isset($_POST['id_xe'])) {
    ...
    $slug = mysqli_real_escape_string($connect, $_POST['slug']);
    $dongxe = mysqli_real_escape_string($connect, $_POST['dongxe']);
    ...
    if (in_array($fileextension, $allowed_extentions)) {
        if ($uploaderrors === 0) {
            ...
            if (move_uploaded_file($templocation, $file_destination)) {
                $connection = "INSERT INTO list_car (id_xe, slug, dongxe, slogan, name, description1, description2, description3, description4, image) VALUES ('$id_xe', '$slug', '$dongxe', '$slogan', '$name', '$description1', '$description2', '$description3', '$description4', 'http://localhost:3000/images/admin/car/$new_file_name')";
